...
def parse_args():
    ...
    all_providers = [
        'Tensorrt', 'CUDA', 'MIGraphX', 
        'ROCM', 'OpenVINO', 'Dnnl', 
        'Nuphar', 'VitisAI', 'Nnapi', 
        'ArmNN', 'ACL', 'Dml', 
        'Rknpu', 'CPU'
    ]
    ...
    parser.add_argument("--engine", type=str, default='ONNX', choices=['ONNX', 'Paddle'])

    # params for ONNX prediction engine
    parser.add_argument("--providers", type=str, default='', help='provider1, provider2' in str(all_providers))

    # params for Paddle prediction engine
    ...
    parser.add_argument("--cls_batch_num", type=int, default=16)
    ...
    if args.engine == 'ONNX':
        ...
        if len(args.providers) == 0:
            args.providers = onnxruntime.get_available_providers()

        print('ONNXRunTime Available Providers: ', onnxruntime.get_available_providers())
        print('Providers in use: ', args.providers)

    return args


def load_onnx_model(args, mode):
    import onnxruntime
    if mode == "det":
    ...
    if 'DmlExecutionProvider' in args.providers:
        print('Disable mem_pattern because using the DmlExecutionProvider.')
        sess_options.enable_mem_pattern = False

    session = onnxruntime.InferenceSession(model_dir, providers=args.providers, sess_options=sess_options)
