...
from utility import get_image_file_list, check_and_read_gif, parse_args
...
class TextRecognizer(object):
    def __init__(self, args):
        ...
        if self.engine=='Paddle':
            ...
            from utility import load_onnx_model
            self.session, self.input_names, self.output_names = \
                load_onnx_model(args, 'rec')
            ...
            if self.rec_algorithm == "SRN":
                ...
                if self.engine=='Paddle':
                    ...
                    outputs = self.session.run(
                        self.output_names, 
                        {input_name: input_data for input_name, input_data in zip(self.input_names, inputs)}
                    )
                ...
                if self.engine=='Paddle':
                    ...
                    outputs = self.session.run(
                        self.output_names, 
                        {self.input_names[0]: norm_img_batch}
                    )
                preds = outputs[0]
            if self.engine=='Paddle':
